...
npm run data -- ../open-doc
wdl deploy .
```

或把三个变量写进 `.env`（参考 `.env.example`，`.env` 已被 gitignore）。
`npm run deploy` 会先跑一次 data 再部署（默认从 `../open-doc` 读）。

其他命令：`wdl whoami` / `wdl workers` / `wdl tail jinshuju-open-api-docs`。

## 在线运行的安全边界

`POST /_proxy` 是纯转发，服务端不存任何凭据：

- 只允许转发到 `https://jinshuju.net`，路径必须以 `/api/v1/` 开头
- 方法白名单 GET / POST / PUT / PATCH / DELETE
- 拒绝路径里的 `..` 和换行符
- 请求体与响应体上限 512 KB
- API Key / Secret 只随单次请求转发，不写日志、不落库；浏览器侧存 `sessionStorage`，关标签页即清除

这是个公开页面，代理端点谁都能调。挂正式域名前建议加限流或登录门槛，改 `src/index.js` 的 `proxy()`。

另外，「请求代码」会把填入的真实 Key / Secret 内联进代码方便直接跑，所以复制出去的片段含明文凭据，别贴到公共渠道。
